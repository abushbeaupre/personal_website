---
title: "Understanding and Interpreting Interactions in Regression Models"
description: "A tutorial on marginal effects, Simpson's Paradox, and sampling bias using the marginaleffects package in R"
author:
  - name: Allen Bush-Beaupré
    url: https://abushbeaupre.github.io/
    orcid: 0000-0001-6989-9278
date: 02-25-2026
categories: [tutorial, interactions, marginaleffects, glmm, statistics, R]
citation:
  url: https://abushbeaupre.github.io/posts/interactions_tutorial/
draft: false
editor: source
format:
  html:
...
Interactions in regression models (GLMM or GAMM) can be confusing to conceptualize and interpret especially when link functions are involved. Often, the typical model coefficients output does not answer the research question that we are interested in.

We will explore various ways interactions can be interpreted depending on a variety of research questions. We will start with linear models and build up to generalized linear models and generalized additive models. Using the marginaleffects package, we will compute various quantities of interests and/or model predictions.
...
# Case Study: Interactions between a categorical and numerical variable
...
avg_slopes(mod_mass_true, variables = "snow_depth", vcov = TRUE, newdata = bighorn_population)
```

The true average slope across the population is `r mean(c(male_loss, female_loss))` kg/cm, which our model recovers accurately.
...
Mathematically, for each individual $i$: $$\text{slope}_i = \frac{\partial E[Y_i]}{\partial X_i}$$

Then we average: $\overline{\text{slope}} = \frac{1}{n}\sum_{i=1}^{n} \text{slope}_i$
...
ggplot() +
  geom_ribbon(data = avg_pred_mass_1, aes(x = snow_depth, ymin = conf.low, ymax = conf.high), color = "lightgreen", fill = "lightgreen", alpha = 0.4) +
  geom_line(data = avg_pred_mass_1, aes(x = snow_depth, y = estimate), color = "lightgreen", lineend = "round", linewidth = 2) +
  geom_ribbon(data = pred_sex_mass_1, aes(x = snow_depth, ymin = conf.low, ymax = conf.high, color = sex, fill = sex), alpha = 0.8) +
  geom_line(data = pred_sex_mass_1, aes(x = snow_depth, y = estimate, color = sex), lineend = "round", linewidth = 2) +
  geom_point(data = bighorn_mass_1, aes(y = weight, x = snow_depth, color = sex)) +
  guides(alpha = "none", fill = "none") +
  labs(title = "Sample predictions (n = 100, balanced)") +
...
Notice that the marginal prediction line is not simply the average of the two conditional lines - it's weighted by the distribution of snow depths for each sex in our sample.
...
ggplot() +
  ...
  guides(alpha = "none", fill = "none") +
  labs(title = "True population (n = 1000)") +
...
ggplot() +
  geom_ribbon(data = avg_pred_mass_2, aes(x = snow_depth, ymin = conf.low, ymax = conf.high), color = "lightgreen", fill = "lightgreen", alpha = 0.4) +
  geom_line(data = avg_pred_mass_2, aes(x = snow_depth, y = estimate), color = "lightgreen", lineend = "round", linewidth = 2) +
  geom_ribbon(data = pred_sex_mass_2, aes(x = snow_depth, ymin = conf.low, ymax = conf.high, color = sex, fill = sex), alpha = 0.8) +
  geom_line(data = pred_sex_mass_2, aes(x = snow_depth, y = estimate, color = sex), lineend = "round", linewidth = 2) +
  geom_point(data = bighorn_mass_2, aes(y = weight, x = snow_depth, color = sex)) +
  guides(alpha = "none", fill = "none") +
