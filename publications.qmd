...
# To update: run process_orcid_simple.R to fetch latest from ORCID
if (file.exists("orcid_publications.rds")) {
  pubs_list <- readRDS("orcid_publications.rds")
  
  # Convert to data frame
  pubs_data <- bind_rows(pubs_list)
  
  # Add bioRxiv links for known preprints
...
function filterTOC(lang) {
  ...
  tocItems.forEach(li => {
    ...
    const hasEnglish = targetElement.classList.contains('lang-en');
    const hasFrench = targetElement.classList.contains('lang-fr');
    
    if (hasEnglish || hasFrench) {
      // This is a language-specific section
      ...
      // This is a non-language-specific section, always show
      li.style.display = 'list-item';
    }
  });
}
