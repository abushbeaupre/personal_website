...
bold_my_name <- function(authors, my_name) {
  ...
  # Bold variations of the author name
  authors <- str_replace_all(authors, "Allen Bush-Beaupré", "<strong>Allen Bush-Beaupré</strong>")
  authors <- str_replace_all(authors, "Allen Bush‐Beaupré", "<strong>Allen Bush-Beaupré</strong>")
  return(authors)
}

# Load publications from automatically generated RDS file
# To update: run debug_orcid.R then process_orcid_with_preprints.R to fetch latest from ORCID
if (file.exists("orcid_publications_full.rds")) {
  all_pubs <- readRDS("orcid_publications_full.rds")

  # Convert published articles to data frame
  if (length(all_pubs$published) > 0) {
    pubs_data <- bind_rows(all_pubs$published)

    # Add bioRxiv links for known preprints of published articles
    pubs_data$biorxiv <- NA_character_
    pubs_data$biorxiv[pubs_data$doi == "10.1093/jee/toaf255"] <- "10.1101/2025.05.09.653057"
    pubs_data$biorxiv[pubs_data$doi == "10.1111/eea.13468"] <- "10.1101/2023.08.01.551352"
    pubs_data$biorxiv[pubs_data$doi == "10.4039/tce.2023.21"] <- "10.22541/au.166862709.99325814/v1"

    # Add abstract field (empty for now)
    pubs_data$abstract <- NA_character_

    # Bold my name
    pubs_data$authors <- sapply(pubs_data$authors, bold_my_name, my_name = my_name)

    # Convert year to numeric for sorting
    pubs_data$year <- as.numeric(pubs_data$year)

    # Sort by year (most recent first)
    pubs_data <- pubs_data %>% arrange(desc(year))
  } else {
    pubs_data <- NULL
  }

  # Convert preprints to data frame
  if (length(all_pubs$preprints) > 0) {
    preprints_data <- bind_rows(all_pubs$preprints)

    # Add abstract field (empty for now)
    preprints_data$abstract <- NA_character_
    
    # Add website field (empty for now)
    preprints_data$website <- NA_character_
    preprints_data$website[preprints_data$doi == "10.64898/2026.02.09.704914"] <- "https://abushbeaupre.github.io/indirect_viz/"

    # Bold my name
    preprints_data$authors <- sapply(preprints_data$authors, bold_my_name, my_name = my_name)

    # Convert year to numeric for sorting
    preprints_data$year <- as.numeric(preprints_data$year)

    # Sort by year (most recent first)
    preprints_data <- preprints_data %>% arrange(desc(year))
  } else {
    preprints_data <- NULL
  }

} else if (file.exists("orcid_publications.rds")) {
  # Fallback to old format
  pubs_list <- readRDS("orcid_publications.rds")
  pubs_data <- bind_rows(pubs_list)
  pubs_data$authors <- sapply(pubs_data$authors, bold_my_name, my_name = my_name)
  pubs_data$year <- as.numeric(pubs_data$year)
  pubs_data <- pubs_data %>% arrange(desc(year))
  preprints_data <- NULL
} else {
  # Fallback: Manual publication list
  pubs_data <- list(
    list(
    ...
    list(
      title = "The effect of sex ratio and group density on the mating success of two lines of Delia platura (Diptera: Anthomyiidae)",
      authors = "Allen Bush-Beaupré, Marc Bélisle, Anne-Marie Fortier, François Fournier, Jade Savage",
      year = "2023",
      journal = "The Canadian Entomologist",
      doi = "10.4039/tce.2023.21",
      biorxiv = "10.22541/au.166862709.99325814/v1"
    )
  ) %>%
    bind_rows() %>%
  ...
  preprints_data <- list(
    list(
      ...
      authors = "Allen Bush-Beaupré, Simon Coroller-Chouraki, Marc Bélisle",
      year = "2026",
      journal = "",
      doi = "10.64898/2026.02.09.704914",
      website = "https://abushbeaupre.github.io/indirect_viz/"
    ),
    list(
      title = "Enemy mitigation in farmlands: Intensive agriculture impacts avian nesting and subsequent ectoparasite infestation",
      authors = "Simon Coroller-Chouraki, Allen Bush-Beaupré, Jade Savage, Marc Bélisle",
      year = "2026",
      journal = "",
      doi = "10.64898/2026.02.02.703377",
...
```{r display-preprints-en}
#| results: asis
if (!is.null(preprints_data) && nrow(preprints_data) > 0) {
  for (i in 1:nrow(preprints_data)) {
    pub <- preprints_data[i, ]
    ...
    details <- c()
    if (!is.na(pub$year)) details <- c(details, pub$year)
    if (!is.na(pub$journal) && pub$journal != "") details <- c(details, paste0("*", pub$journal, "*"))
    ...
    if (!is.na(pub$doi)) {
      links <- c(links, paste0('[bioRxiv preprint](https://doi.org/', pub$doi, ')'))
    }
    
    if (!is.na(pub$website) && pub$website != "") {
      links <- c(links, paste0('[Publication Website](', pub$website, ')'))
    }
...
## Pré-publications {.lang-fr}

::: lang-fr
```{r display-preprints-fr}
#| results: asis
if (!is.null(preprints_data) && nrow(preprints_data) > 0) {
  for (i in 1:nrow(preprints_data)) {
    pub <- preprints_data[i, ]
    ...
    if (!is.na(pub$doi)) {
      links <- c(links, paste0('[Préimpression bioRxiv](https://doi.org/', pub$doi, ')'))
    }
    
    if (!is.na(pub$website) && pub$website != "") {
      links <- c(links, paste0('[Site web de la publication](', pub$website, ')'))
    }
  ...
  cat("Aucune pré-publication disponible.\n")
}
```
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Filter TOC immediately
  filterTOC(currentLang);
...
// Also filter TOC after window load to ensure it's ready
window.addEventListener('load', function() {
  let currentLang = localStorage.getItem('language') || 'en';
  filterTOC(currentLang);
  setTimeout(() => filterTOC(currentLang), 100);
  setTimeout(() => filterTOC(currentLang), 500);
});
...
function filterTOC(lang) {
  ...
  // Exit if no TOC found
  if (tocItems.length === 0) {
    console.log('No TOC items found');
    return;
  }

  console.log('Filtering TOC for language:', lang);
  console.log('Found', tocItems.length, 'TOC items');

  // Define which sections belong to which language
  const englishSections = ['journal-articles', 'preprints'];
  const frenchSections = ['articles-de-revues', 'publications'];
  ...
  tocItems.forEach(li => {
    ...
    console.log('Processing TOC item with href:', href);

    // Check if this is an English or French section using includes
    ...
    if (lang === 'en' && isFrench && !isEnglish) {
      li.style.setProperty('display', 'none', 'important');
      console.log('Hiding French section:', href);
    } else if (lang === 'fr' && isEnglish) {
      li.style.setProperty('display', 'none', 'important');
      console.log('Hiding English section:', href);
    } else if (isEnglish || isFrench) {
      li.style.setProperty('display', '', 'important');
      console.log('Showing section:', href);
    }
  });
}
</script>
...
/* Language-specific header visibility */
h2.lang-en {
  display: block;
}
